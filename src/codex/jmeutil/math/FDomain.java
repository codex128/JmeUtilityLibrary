...
public class FDomain {
	...
	public static final FDomain
			RADIANS = new FDomain(0f, FastMath.TWO_PI);
	 ...
	 * May be null, in which case there is no minimum.
	 * @param min
	 * @return 
	 */
	...
	/**
	 * Repels the value to outside the minimum and maximum values (whichever is closest).
	 * Only repels if both min and max are not null.
	 * @param value
	...
	/**
	 * Wraps the value within the minimum and maximum values.
	 * If min or max is null, will use <code>-Float.MAX_VALUE</code> or
	 * <code>Float.MAX_VALUE</code> instead, respectively.
	 * <br>
	 * <strong>Known Issue!</strong><br>
	 * Will not work correctly if the value is less than the
	 * minimum minus the range or more than the maximum plus the range.
	 * @param value
	 * @return the wrapped value
	 */
	public float applyWrap(float value) {
		if (min != null && value < min) {
			if (max == null) return Float.MAX_VALUE;
			else return max-(min-value);
		}
		else if (max != null && value > max) {
			if (min == null) return -Float.MAX_VALUE;
	...
	/**
	 * Returns the distance between the minimum and maximum values.
	 * Returns -1f if min or max is null.
	 * @return 
	 */
