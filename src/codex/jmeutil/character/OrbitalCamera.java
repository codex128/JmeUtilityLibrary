...
public class OrbitalCamera extends AbstractControl implements AnalogFunctionListener, StateFunctionListener {
	...
	public static final FunctionId
			ROTATE_XZ = new FunctionId(INPUT_GROUP, "rotate_xz"),
	...
	float horizontalSpeed = FastMath.PI/100f;
	float verticleSpeed = FastMath.PI/150f;
	float camspeed = .25f;
	...
	public OrbitalCamera(Camera cam, InputMapper im) {
		setCamera(cam);
		if (!inputInitialized) initializeInputMappings(im);
		listenForInput(im);
	}
	...
	public void valueActive(FunctionId func, double value, double tpf) {
		if (func == ROTATE_XZ) {
			rotateHorizontally((float)value);
		}
		else if (func == ROTATE_Y) {
			rotateVertically(-(float)value);
		}
	}
	@Override
	 ...
	 * Often used to snap the camera location immediately to the ideal camera location.
	 * @param location 
	 */
	...
	/**
	 * Halts the location base from being updated.
	 * The camera will always be anchored to the same location,
	 * no matter where the subject moves. The camera is still able
	 * to orbit around the location base (unless the camera location
	 * itself is frozen).
	...
	/**
	 * Halts the focus base location from being updated.
	 * The camera will always look at that same location,
	 * no matter where the subject moves.
	...
	/**
	 * Halts the camera location from being updated.
	 * The camera will always remain in the same location,
	 * ignoring subject location and horizontal/verticle rotation.
	...
	private Vector3f calculateNextCameraPosition() {
		...
		switch (motion) {
			...
			case LINEAR: return motionLinear(cam.getLocation(), target, camspeed);
			case LERP: return motionLerp(cam.getLocation(), target, camspeed);
			default:
	...
	/**
	 * Calculates the ideal location of the camera.
	 * Does not represent the true location of the camera.
	 * @return 
	 */
	...
	/**
	 * The offset from the spatial the camera will look at.
	 * default=(0,0,0)
	 * @param vec 
	 */
	...
	/**
	 * The offset from ideal location the camera will be.
	 * default=(0,0,0)
	 * @param vec 
	 */
	...
	/**
	 * The horizontal angle (xz) the ideal location is at.
	 * default=0f
	 * @param angle 
	 */
	...
	/**
	 * The verticle angle (y) the ideal location is at.
	 * default=0f
	 * @param angle 
	 */
	...
	/**
	 * The distance the camera is from the spatial.
	 * default=5f
	 * @param distance 
	 */
	...
	/**
	 * Sets the mode of motion the camera uses to move toward the ideal location.
	 * default=<code>Motion.INSTANT</code>
	 * @param motion 
	 */
	...
	/**
	 * Enables dynamic distancing.
	 * Dynamic distancing continually updates the ideal camera distance
	 * to match the real camera distance.
	 * default=false
	 * @param enable 
	 */
	...
	/**
	 * Enables trailing mode.
	 * Trailing mode disables the effect of horizontal rotation. The camera will
	 * move straight at the spatial (plus location offset). Does apply verticle rotation.
	 * 
	 * default=false
	 * @param enable 
	 */
	...
	/**
	 * The domain of the verticle angle.
	 * default=[min:0f, max:PI/2]
	 * @return 
	 */
	...
	/**
	 * The domain of the camera distance.
	 * default=[min:3f, max:7f]
	 * @return 
	 */
